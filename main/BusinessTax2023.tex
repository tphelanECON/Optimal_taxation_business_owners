...
\iffalse
Concern: is Assumption 4.1 satisfied for the example I compute in the earlier section? 
\fi
...
%The stationary distribution of a geometric Brownian motion that is killed at a constant rate and reinjected at some $\overline{X}$ has a stationary distribution of the double-Pareto form $f(x) = A x^{\alpha^+_X-1}1_{x \leq \overline{X}} + B x^{\alpha^-_X-1}1_{x > \overline{X}}$ for some $A,B > 0$ and tail parameters $\alpha^{\pm}_X$.
...
\begin{figure}[!htb]
\centering
\includegraphics[width=0.49\linewidth]{cdf_c_main}
\includegraphics[width=0.49\linewidth]{cdf_theta_main}
...
\iffalse
Simple guess-and-verify for the worker. Their HJB is given by
\begin{align*}
\rho V_W(a) & = \max_{c}\rho \frac{c^{1-\overline{\gamma}}}{1-\gamma} + [\rho a - ca + wL]V_W'(a).
\end{align*}
Now assume $V_W(a) = \overline{V}_W(a + wL/\rho)^{1-\overline{\gamma}}/(1-\gamma)$ for some $\overline{V}_W$. Also assume that $c(a) = \overline{c}(a+ wL/\rho)$ for some $\overline{c}$. Then substitution gives
\begin{align*}
\rho \overline{V}_W \frac{(a + wL/\rho)^{1-\overline{\gamma}}}{1-\gamma} & = \max_{\overline{c}}\rho \overline{c}^{1-\overline{\gamma}}\frac{(a + wL/\rho)^{1-\overline{\gamma}}}{1-\gamma} 
\\ & + (1-\alpha)[\rho a - \overline{c}(a+ wL/\rho) + wL]\overline{V}_W(a + wL/\rho)^{-\overline{\gamma}}.
\end{align*}
Rearrangement then gives 
\begin{align*}
\frac{\rho \overline{V}_W}{1-\gamma} & = \max_{\overline{c}}\rho \frac{\overline{c}^{1-\overline{\gamma}}}{1-\gamma}  + (1-\alpha)[\rho  - \overline{c}]\overline{V}_W.
\end{align*}
The first-order condition implies $\overline{c} = (\rho/\overline{V}_W)^{1/\overline{\gamma}}$. Substitution then gives
\begin{align*}
\frac{\rho \overline{V}_W}{1-\gamma} & = \frac{\rho}{1-\gamma} (\rho/\overline{V}_W)^{1/\overline{\gamma}-1} + (1-\alpha)[\rho  - (\rho/\overline{V}_W)^{1/\overline{\gamma}}]\overline{V}_W
\\ \frac{\rho}{1-\overline{\gamma}} & = \frac{\rho^{1/\overline{\gamma}}}{1-\overline{\gamma}} (\overline{V}_W)^{-1/\overline{\gamma}} + \rho  - (\rho/\overline{V}_W)^{1/\overline{\gamma}}
\\ \frac{\rho \overline{\gamma}}{1-\overline{\gamma}} & = \frac{\overline{\gamma}}{1-\overline{\gamma}} (\rho/\overline{V}_W)^{1/\overline{\gamma}}
\end{align*}
and so the coefficient satisfies $(\rho/\overline{V}_W)^{1/\overline{\gamma}} = \rho$ and hence $\overline{V}_W = \rho^{1-\overline{\gamma}}$. It follows that 
\begin{align*}
V_W(a) & = \overline{V}_W \frac{(a + wL/\rho)^{1-\overline{\gamma}}}{1-\gamma}
\\ & = \rho^{1-\overline{\gamma}} \frac{(a + wL/\rho)^{1-\overline{\gamma}}}{1-\gamma}
\end{align*}
\fi
...
\begin{align*}
%d\theta_t & = \mu_{\theta}\theta_tdt + \sigma_{\theta} \theta_t dZ_t
...
%I suspect that the only wrinkle here is that we need to make sure that we get $\overline{c}$ and $l$ defined on the right grids. 
...
%\\ & = \frac{A}{\alpha^+_X} {\left[x^{\alpha^+_X}\right]}_{0}^{\overline{X}} + \frac{B}{\alpha^-_X} {\left[x^{\alpha^-_X}\right]}_{\overline{X}}^{\infty}
...
\label{B}
\end{equation}%Substitution gives the density 
...
In this appendix I derive a general expression for the optimal taxes on interest and wealth under the assumption $\overline{\mu}_0 = \rho$ maintained at the end of Section \ref{Imp}. Recall that under this assumption the first-order condition characterizes the optimal retention choice and solves %multiplying through by $\tilde{x}(\overline{\iota};\tau) \times \overline{\iota}$
$$
0 = \frac{[1 + \alpha(\gamma-1)]}{(\gamma-1)(1 - 2\alpha)}\tilde{x}'(\overline{\iota};\tau)\overline{\iota} + \tilde{x}(\overline{\iota};\tau).
$$
...
\end{equation} %$\sigma_c/\sigma = [(1/\alpha-1)l]^{-1}$
...
%I don't think it matters for the purpose of the calculation of the stationary distribution whether or not 
...
\end{aligned}
\label{usual}
\end{equation} %this is no different from what Young does though. For the solution to the individual problem, I used the fact that the value function at the boundary points was known. For the calculation of the stationary distribution,
together with the assumption that the boundary points are absorbing. It remains to choose the probabilities $p^{(2)}$ that form a locally consistent chain for %(avoid any superscripts)
\begin{equation}
...
\iffalse 
I now compute several stationary distributions for both the efficient and restricted-action allocations. In Figure \ref{fig:RC_pre}, I first depict the average productivity and consumption of entrepreneurs, the functions $M$ and $C$ defined in \eqref{Output}, in both the efficient and optimal restricted-action allocations, where the parameters are fixed at those in Section \ref{PriAg}. I earlier emphasized that the difference between the efficient allocation and the optimal restricted-action allocation vanishes as normalized utility fallsand so the dotted and solid lines coincide near the origin. Further, the solution to \eqref{STATy} necessarily tends to zero as the fraction of entrepreneurs become small, which reflects an intuitive point: as entrepreneurs become rarer (and hence more valuable), the efficient allocation becomes well-approximated by an allocation in which they work as hard as possible indefinitely, regardless of their history of productivity shocks.

%Notice that the consumption of entrepreneurs need not be increasing in normalized utility, because as utility rises the planner may compensate the agents more with leisure than consumption. However, this need not lead to multiplicity of solutions to the resource constraints, because entrepreneurs are only one part of the population and workers' consumption is increasing in normalized utility.

%[!htb]
\begin{figure} %[H]
\centering
\includegraphics[width=0.49\linewidth]{M}
\includegraphics[width=0.49\linewidth]{C}
%\includegraphics[width=0.49\linewidth]{RC}
\caption{Output and consumption per entrepreneur and leisure.}\label{fig:RC_pre}
\end{figure}%(the left- and right-hand sides of equation \eqref{STATy})
\fi
...
\begin{equation}
\overline{p} = \frac{\Delta_t}{\Delta_u^2} \frac{\sigma_u^2}{2}. 
\label{1stcond}
\end{equation}
This actually corresponds to the canonical finite-difference construction.
...
\iffalse
\textbf{Five-point approximation.} The above is the simplest way in which to construct a locally consistent chain, but it can lead to an inaccurate solution, especially for low $u$ when the . We can make things more accurate by allowing multiple different transitions. When there are four possible transitions, the relevant pairs are $(m_{11},m_{12})$ and $(m_{21},m_{22})$. The first pair $(m_{11},m_{12})$ is chosen as above. Now define 
\begin{equation} %m_1 \leq \overline{m} \right\}}
z := {\left| m^*_1 - \frac{|\sigma_uu|/\Delta_u}{\sigma/\Delta_y} \right|}.
\label{z4}
\end{equation}
We 


I want to keep things simple. I will use the fact that $\sigma_u < 0$ everywhere in this environment. If 

%Proceeding in this manner gives us a density over $(u, y)$, written $f(u,y)$. We now want to obtain the density over $y$ and then $\theta$. Summing over the first dimension gives us the density over $y$. 


\begin{align*}
d\theta_t & = \mu_{\theta}(u_t)\theta_tdt + \sigma_{\theta}(l(u_t)) \theta_tdZ_t
\\ dy_t & = {\left(\mu_{\theta}(u_t) -  \sigma_{\theta}(l(u_t))^2/2 \right)}dt + \sigma_{\theta}(l(u_t)) dZ_t = \mu_y(u_t)dt + \sigma_y(u_t)dZ_t. 
\end{align*}
\fi

where $\mu_u$ and $\sigma_u$ are given by % (writing as a function of $u$),
\begin{equation}
\begin{aligned}
\mu_u(\overline{c},l) & = \rho {\left(\frac{1 - (\overline{c}^{1-\alpha}l^{\alpha})^{1-\gamma}}{1-\overline{\gamma}}\right)}  + (\overline{\gamma}-1)\frac{\sigma^2}{2} E(l)^2(\overline{c}^{1-\alpha}l^{\alpha})^{2-2\gamma} 
\\ & + \frac{1}{2}(\sigma E(l)(\overline{c}^{1-\alpha}l^{\alpha})^{1-\gamma}-\sigma_{\theta}(l))^2 - \mu_{\theta}(l) + \frac{\sigma_{\theta}^2(l)}{2}
\\ \sigma_u(\overline{c},l) & = \sigma E(l)(\overline{c}^{1-\alpha}l^{\alpha})^{1-\gamma} - \sigma_{\theta}(l).
\label{recall_mu2}
\end{aligned} 
\end{equation} % and $w_t := f(u_t)$
Due to the fat-tailed distribution of $\theta$, it will be convenient to change variables to $y_t := f(\theta_t) := \ln \theta_t$. Using $f'(\theta) = 1/\theta$ and $f''(\theta) = -1/\theta^2$, Ito's Lemma implies that 
\begin{align*}
d\theta_t & = \mu_{\theta}(u_t)\theta_tdt + \sigma_{\theta}(l(u_t)) \theta_tdZ_t
\\ df(\theta_t) & = {\left(\mu_{\theta}(u_t)\theta_tf'(\theta_t) + \frac{1}{2}\sigma_{\theta}(l(u_t))^2 \theta_t^2f''(\theta_t)  \right)}dt + \sigma_{\theta}(l(u_t)) \theta_t f'(\theta_t) dZ_t,
\end{align*}
and so the system we consider becomes 
\begin{equation}
\begin{aligned}
du_t & = \mu_u(\overline{c}(u_t),l(u_t))u_tdt + \sigma_u(\overline{c}(u_t),l(u_t)) u_tdZ_t
\\ dy_t & = \mu_y(u_t)dt + \sigma_y(u_t)dZ_t
\end{aligned}
\label{final_sys}
\end{equation}
where 
\begin{align*}
(\mu_y(u), \sigma_y(u)) & = (\mu_{\theta}(u) -  \sigma_{\theta}(l(u))^2/2, \sigma_{\theta}(l(u))).
\end{align*}
